...
astSemantics.addOperation<AST.ASTNode>("toAST", {
  ...
  ClassStatement(
    ...
    return new AST.ClassStatement(
      this.getLoc(),
      className.getSymbol(),
      parentName.sourceString === "" ? null : parentName.toAST(),
  ...
  Attribute(name, _COLOR, tyepDecl, _ARROW, init) {
      const actualInit = init.sourceString === "" ? new AST.NoExpr(this.getLoc()) : init.toAst();
  ...
  Formal(name, _COLON, typeDecl) {
      return new AST.Formal(this.getLoc(), name.getSymbol(), typeDecl.getSymbol());
  },
  ...
  Expr_LessThanOrEqual(lhs, _LT, rhs) {
  ...
  DispatchExpression(expr) {
    console.log("here", this.ctorName, this.sourceString);
    return expr.toAST();
  },

  DispatchExpression_Dynamic(caller, _DOT, methName, _LP, args, _RP) {
    const actualCaller = caller.sourceString === ""
      ...
      console.log("here", this.ctorName);
    
    return new AST.DynamicDispatch(
      this.getLoc(),
      actualCaller,
      methName.getSymbol(),
      args.children.map((e) => e.toAST()),
    );
  },
