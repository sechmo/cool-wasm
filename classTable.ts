...
import { Attribute, ClassStatement, Formal, Method, NoExpr, type Classes } from "./ast.ts";
...
export class ClassTable {
    ...
    private addClass(cls: ClassStatement): void {
        if (this.classExists(cls.name)) {
            ErrorLogger.semantError(cls.location, `Class ${cls.name.getString()} is already defined`);
            return;
        }

        if (cls.name === ASTConst.SELF_TYPE) {
            ErrorLogger.semantError(cls.location, `"${ASTConst.SELF_TYPE.getString()}" cannot be used as a class name`)
            return;
        }
    ...
    private isSubclassNoSelfType(child: AbstractSymbol, parent: AbstractSymbol): boolean {
        let currCls = child;
    ...
    public isSubclass(child: AbstractSymbol, parent: AbstractSymbol, currCls: AbstractSymbol): boolean {
        if (child === ASTConst.SELF_TYPE && parent === ASTConst.SELF_TYPE) return true;

        if (parent === ASTConst.SELF_TYPE) return false;

        if (child === ASTConst.SELF_TYPE) return this.isSubclassNoSelfType(currCls, parent);
        
        return this.isSubclassNoSelfType(child, parent);
    }

    private leastUpperBoundNoSelfType(clsA: AbstractSymbol, clsB: AbstractSymbol): AbstractSymbol {
    ...
    public leastUpperBound(clsA: AbstractSymbol, clsB: AbstractSymbol, currCls: AbstractSymbol): AbstractSymbol {
        if (clsA === ASTConst.SELF_TYPE && clsB === ASTConst.SELF_TYPE) return ASTConst.SELF_TYPE;

        if (clsA === ASTConst.SELF_TYPE) return this.leastUpperBoundNoSelfType(currCls, clsB);
        if (clsB === ASTConst.SELF_TYPE) return this.leastUpperBoundNoSelfType(currCls, clsA);
    ...
    private checkValidInheritance(): void {
        for (const [child, parent] of this.childToParent.entries()) {
            ...
            if (parent === ASTConst.Bool) {
                ErrorLogger.semantError(this.classNode(child).location, "cannot inherit from Bool")
                continue;
            }


            if (parent === ASTConst.Int) {
                ErrorLogger.semantError(this.classNode(child).location, "cannot inherit from Int")
                continue;
            }


            if (parent === ASTConst.Str) {
                ErrorLogger.semantError(this.classNode(child).location, "cannot inherit from Str")
                continue;
            }
            
            if (parent === ASTConst.SELF_TYPE) {
                ErrorLogger.semantError(this.classNode(child).location, "cannot inherit from SELF_TYPE")
                continue;
            }

            if (!this.classExists(parent)) {
                ErrorLogger.semantError(this.classNode(child).location, `cannot inherit from undefined class ${parent.getString()}`)
                continue;
            }
        ...
        for (const cls of this.childToParent.keys()) {
            ...
            while (currCls != ASTConst.Object_) {
                ...
                if (seenClasses.has(currCls)) {
                    ErrorLogger.semantError(this.classNode(cls).location, `class ${cls.getString()}: cyclic class inheritance with class (${currCls.getString()})`)
                    break;
                }
            }
        }
    ...
    private installBasicClasses(): void {
        ...
        const ObjCls = new ClassStatement(
            ...
            ASTConst.No_class,
            [
                new Method(loc, ASTConst.cool_abort, [], ASTConst.Object_, new NoExpr(loc)),
        ...
        const IOCls = new ClassStatement(
            ...
            ASTConst.Object_,
            [
                new Method(loc, ASTConst.out_string, [new Formal(loc, ASTConst.arg, ASTConst.Str)], ASTConst.SELF_TYPE, new NoExpr(loc)),
                new Method(loc, ASTConst.out_int, [new Formal(loc, ASTConst.arg, ASTConst.Int)], ASTConst.SELF_TYPE, new NoExpr(loc)),
        ...
        const StrCls = new ClassStatement(
            ...
            [
                ...
                new Attribute(loc, ASTConst.str_field, ASTConst.prim_slot, new NoExpr(loc)),
                new Method(loc, ASTConst.length, [], ASTConst.Int, new NoExpr(loc)),
                new Method(loc, ASTConst.concat, [new Formal(loc,ASTConst.arg, ASTConst.Str)], ASTConst.Str, new NoExpr(loc)),
                new Method(loc, ASTConst.substr, [new Formal(loc,ASTConst.arg, ASTConst.Int),new Formal(loc,ASTConst.arg2, ASTConst.Int)], ASTConst.Str, new NoExpr(loc)),
            ]
        );
